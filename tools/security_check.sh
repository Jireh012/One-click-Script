#!/bin/bash
# Usage:
#   wget --no-check-certificate https://raw.githubusercontent.com/Jireh012/One-click-Script/main/tools/security_check.sh && chmod +x security_check.sh && sh security_check.sh

#=================================================
#	System Required: CentOS/Debian/Ubuntu
#	Description: SecurityCheckTools
#	Version: 1.0.0
#	Author: Jireh
#	Blog: https://www.lyile.cn/
#=================================================

echo "#注意:本脚本只是检查,未对服务器做任何修改,安全人员可根据结果进行相应安全整改 #"
echo " "
echo " "
echo "##########################################################################"
echo "#                                                                        #"
...
if [ $? == 0 ];then
  echo "警告：发现危险进程kingsing,请进行安全排查！！！"
else
  echo "未发现kingsing可疑进程"
fi
ps -aux | grep kdevtmpfsi | grep -v grep
if [ $? == 0 ];then
  echo "警告：发现危险进程kdevtmpfsi，请进行安全排查。"
else 
  echo "未发现kdevtmpfsi可疑进程"
fi
echo ">>>>>>亡命徒（outlow）僵尸网络攻击排查:"
ps -aux | grep kswapd0 | grep -v grep
if [ $? == 0 ];then
  echo "警告：发现可疑进程kswapd0,请结合资源占用与远程连接情况进行排查"
else
  echo "未发现kswapd0可疑进程"
fi
ps -aux | grep rsync | grep -v grep
if [ $? == 0 ];then
  echo "警告：发现危险远程连接进程rsync，请进行安全排查。"
else 
  echo "未发现远程连接可疑进程"
fi
ps -aux | grep tsm | grep -v grep
if [ $? == 0 ];then
  echo "警告：发现危险扫描进程tsm,请进行安全排查。"
else
  echo "未发现扫描可疑进程"
fi
echo "=============================恶意IP排查================================"
dos2unix Dangerous_ip.txt
echo ">>>>>>正在根据Dangerous_ip文件进行ip检查"
for ip in `cat Dangerous_ip.txt`
do 
  netstat -antp | grep $ip && echo "发现可疑IP连接"
done
echo ">>>>>>OK，检查完毕"
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>end<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<"